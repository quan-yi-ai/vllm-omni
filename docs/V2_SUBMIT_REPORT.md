# v2 提交验证报告 — 22 项管线优化全量落地版（910B2 实测）
...
**与 910C 参考数据的差距属硬件差异**：同分支代码此前在昇腾 910C（Atlas A3）
实测 zh2020 全量
...
1. **K=12 错位崩溃修复**：`npu_ar_model_runner.py` `_talker_local_steps` 8→12。
   根因：早前同步的快照 `ea66d2d2`（"clean tree"）含 scheduler 12 / runner 8
   的 KV 窗口错位 → speech warmup 崩溃。以其
...
（99.2 vs 118.0ms）；E2EL/RTF/TTFP 差距与 910B2/910C 单 die 算力差一致（stage2 在
910B2 上 kernel-launch 占比更高）。
...
3. **历史版本甄别原则**：早期快照 `ea66d2d2` 存在 K12 误回退，一切以其
