...
> 状态：**方案设计**（未实施）。合规依据：官方 FAQ 9.1 原文——
> **"允许对 MiniCPM-o 4.5 的 Token2Wav/Flow 模块进行蒸馏或微调、替换官方权重，
> 但优化后模型必须通过全部精度准入门槛。"**
> 即：允许范围限定为 Token2Wav/Flow 模块（`flow.pt` / `hift.pt`），
> Thinker LLM 与 Talker AR 权重**不在允许范围**。本方案只蒸馏 `flow.pt`
> 内的 decoder DiT（114.6M），完全合规；且"替换官方权重"确认了
> strict=True 整文件替换的交付路线可行。
>
> 另据 9.2：允许投机推理及执行侧任意优化（调度/缓存/算子/计算图等）。
> 头部队伍（No.1 RTF 0.1546，基线 35%）几乎必然已走此路线。
>
