...
| **清理后代码终验（a844f0b7，32 样本）** | — | **RTF 0.5238 / WER 0.6167%** | 与 0.5174 波动 ±1.2%，质量更优 |
| **累计（基线 → 最终）** | **~0.57** | **0.5238（终验）/ 0.5174（最优轮）** | **−8.1%/−9.5%** |
...
| **Daily-Omni 视频问答（§13，侧验证）** | — | **77.84%**（801/1029） | 与 qwen3-omni 官方（~76–77%）持平，多模态链路无回归 |

第二轮实验（icf=4 / thinking-off）为负优化已回退，见 §12；全部优化已固化为
提交链 `b268007c → 0fc86a6f → 30860772 → e9bb319d → 5804b7fe → a844f0b7 → 655b05e3`（branch
`minicpm-challenge`，HEAD=655b05e3，工作区干净）。

清理后代码（a844f0b7）终验：synth 95.68s / audio 182.68s → RTF **0.5238**，WER
**0.6167%**（32 样本，seed=1）——与 0.5174 的差异属 run-to-run 波动（NPU 时钟/
热状态），清理提交本身零行为变更。
...
## 13. Daily-Omni 视频问答侧验证（多模态链路无回归）

> 目的：TTS 赛道的优化（EOS 修复、chat template、stage 配置注入、NPUGraph 等）
> 是否会误伤模型的其他能力？用一条**与 TTS 无关的完整多模态链路**（看视频→听问题→
> 生成回答）做交叉验证。

### 13.1 评测设置

- 基准：Daily-Omni（多模态视频 QA），原始 1196 条记录，按 `(video_id, Answer)`
  去重后 **1029 个唯一样本**；
- 分片运行：`/workspace/dailyomni_shard_run.py`（shard 化评测，防单进程 OOM，
  分片间 `/dev/shm` 清理）；
- 评分：按基准官方答案比对，输出逐条对错 + 汇总准确率。
...
| 正确 | **801** |
| **准确率** | **77.84%** |
| 参照（qwen3-omni 官方报告同基准） | ~76–77% |

**结论：持平参照系。** 我们提交链上的多模态理解/生成链路能力与 qwen3-omni 量级
模型相当，未观测到工程改动引入的能力回归。
...
| 错误类型 | 数量 | 占比 |
|---|---|
| wrong_answer（内容答错） | 228 | 100% |
| parse_failed（解析失败） | **0** | 0% |
| crash（进程崩溃/超时） | **0** | 0% |

黄金答案分布：A:45 / B:59 / C:63 / D:61——均匀，无多选偏置。

三条证据合起来：错误全部是**模型真实能力边界**，而非提示词泄漏、解析 bug 或
评测不稳定——即 77.84% 是干净的数字，可放心作为侧验证结论。
