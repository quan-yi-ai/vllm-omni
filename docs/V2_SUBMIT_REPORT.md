# v2.1 提交验证报告 — 22 项管线优化 + true-greedy（910B2 实测）

> 基线 HEAD `0864acd4`（22 项优化全量落地 + K12/W4 修复）
> 硬件：**昇腾 910B2 单卡 64GB**（容器 cgroup 32GB 内存）· CANN 9.0.0 · aarch64
> 日期：2026-08-30（v2.1 增补 2026-09-02：Stage1 codec true-greedy）· Seed-TTS zh 官方口径（max_concurrency=1, temperature=0）
...
**与 910C 参考数据的差距属硬件差异**：同分支代码此前在昇腾 910C（Atlas A3）实测
zh2020 全量 E2EL 841.98 / TTFP 236.83 / RTF 0.16606；我们在 910B2 上同口径 40 条
E2EL 994.96→910.91 / TTFP 333 / RTF 0.21。910B2 单 die 算力约为 910C 的一半量级，
且 stage2 在 910B2 上 kernel-launch 开销占比更高（cf.
docs/minicpmo_npu_optimization_report.md §11.1 launch-bound 分析）。**两代硬件对
同一份代码都远超官方基线**（910C −62% RTF，910B2 −55% RTF），证明优化是平台
无关的算法/调度层收益。

---

## 1. 提交内容（v2.1：+ Stage1 codec true-greedy）

### 1.1 22 项管线优化（全量落地，commit bfbc7012）
...
（完整 22 项明细见 `05_optimization_report/README.md` #1-22 表格）
...
### 1.3 Stage1 codec true-greedy（top-k=1，本轮新增）

Stage1（codec LM）采样改为真正的 greedy：`_CODEC_TOP_K = 1`（原 25），并在
`_sample_audio_codes` 中对 top-k=1 走 `logits.argmax(-1)` 快路径（跳过 top-k
排序核与采样核）。temperature=0 时 top-k>1 的候选裁剪本就不影响结果，此改动
只是消除无谓的排序/采样开销，并使输出严格确定性（双跑 PCM 帧数逐帧一致、
RMS 最大偏差 0.2%）。

- 回滚开关：`VLLM_MINICPMO_STAGE1_CODEC_TOP_K=25`（显式 env 优先于内置默认）
- 依据：官方 checkpoint 该层即按 greedy 训练/推理对齐

A/B 实测（40 条官方口径、热轮、同固定样本）：

| 指标 | k=25 基线 | greedy | Δ |
|---|---|---|---|
| E2EL | 1005.26 ms | **973.26 ms** | −3.2% |
| TTFP | 348.11 ms | **326.67 ms** | −6.2% |
| RTF | 0.22 | 0.21 | 持平偏好 |
| WER | 0.0062 | 0.0076 | 噪声级（中位数均 0） |
...
1. **K=12 错位崩溃修复**：`npu_ar_model_runner.py` `_talker_local_steps` 8→12。
   根因：早前合并引入的版本带 scheduler 12 / runner 8 的 KV 窗口错位 →
   speech warmup 崩溃。按 **origin/submit（62f4e4ab）提交版为准**修复。
   修复后 warmup 7.5s PASS。
...
| 910C 参考数据 zh2020 全量 | 841.98 | 236.83 | 0.16606 | 118.0 | 0.00987 | 0.84317 |
...
| 910B2 vs 910C（含硬件差） | +24.0% | +43.3% | +20.4% | **−15.9%** | **−5.8%** | −0.65% |

注：全量口径 910B2 **WER 优于 910C 参考数据**（0.0093 vs 0.00987）、**TTFT 更快**
（99.2 vs 118.0ms）；E2EL/RTF/TTFP 差距与 910B2/910C 单 die 算力差一致
（stage2 在 910B2 上 kernel-launch 占比更高）。
...
3. **历史版本甄别原则**：早期快照 ea66d2d2 存在 K12 误回退，一切以
   origin/submit 62f4e4ab 为准（K12 修复的教训）。
